...
import { PeerRemoteDirectAudio, PeerRemoteMixerAudio } from '../components/consumers/peer_remote'
  ...
  return (
    <div className="w-full h-full grid grid-cols-3 grid-rows-3 gap-4">
      ...
      {remote_peers
        ...
        .map((p) =>
          audio_direct ? <PeerRemoteDirectAudio key={p.peer} peer={p} /> : <PeerRemoteMixerAudio key={p.peer} peer={p} />
        )}
